...
figure
hold on
PlotMap(BW,[0.7 0.7 0.7])           % grey color
...
axis tight
if Ngoals>1
    legend([splot gplot igplot],'start','goal','intermediate goals','location','southwest')
else
    legend([splot gplot],'start','goal','location','southwest')
end
hold off
...
[path,res]=BW2GAZEBO(pathOutput,n);
...
% Compute critical points in the trajectory (turn bigger than 80 deg)

crit_idces=[];
for ii=1:length(path)-3
    ...
    angle=atan2(dir1(1)*dir2(2)-dir1(2)*dir2(1),dot(dir1,dir2));

    if angle>deg2rad(80) || angle<deg2rad(-80)
        crit_idces=[crit_idces ii+1];
    end

end
if isempty(crit_idces)
    crit_idces=-1;
end
...
pause(1)
out=sim('PID_houseROOMBA.slx');
% save(['path_AstarROOMBA_',num2str(simNumber)],'out','path','BW_beforeClear','n','BW','pathOutput','start','goal','i_goals')
...
load(['path_AstarROOMBA_',num2str(simNumber)])
...
res_BW=0.05*384/n;  
xtks=xtks*res_BW-10;
ytks=ytks*res_BW-10.7;   % all to get axes coordinates
xtks=round(xtks,1);
ytks=round(ytks,1);
