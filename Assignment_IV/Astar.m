function points = Astar(BW, G, start_pos, goal_pos, num_image, diag)

% Plotting
plot_map(BW)
hold on;
h_red = plot(goal_pos(1), goal_pos(2), 'ro', 'MarkerFaceColor', 'r');
h_green = plot(start_pos(1), start_pos(2), 'go',...
'MarkerFaceColor', 'g');
...
while any(nodelist(:,1)==1) && act_node~=goal
    ...
    alive = find(nodelist==1); % alive points (to be visited) indexes
    [row_con, col_con] = ind2sub(size(BW), alive); % to see now
    [row_goal, col_goal] = ind2sub(size(BW), goal);
    % Manh: H = max(abs(row_con - row_goal), abs(col_con - col_goal));
    H = sqrt((row_con - row_goal).^2 + (col_con - col_goal).^2);

    [~, act_index] = min(dist(nodelist==1) + H); % 
